...
namespace OfflineFileGenerator
{
    public partial class App
    {
        ...
        public int GenerateStandardXLSMData(string filepath, string savedirectory, SqlDataReader reader)
        {
            ...
            try
            {
                ...
                //using (ExcelPackage pck = new ExcelPackage(newFile, tmpXlsm))
                {
                    ...
                    {
                        if (reg.IsMatch(pck.Workbook.Worksheets[i].Name) || HideConditionSheetIndex == i.ToString())
                ...
                if (File.Exists(tmpFile))
                {
                    ...
                    var StandardDateSheetWithFormControl = ConfigurationManager.AppSettings["StandardDateSheetWithFormControl"];
                    if (StandardDateSheetWithFormControl == "1")
                    {
                        for (int i = 0; i < DateSheetCollection.Count; i++)
                        {
                            var sheetName = string.Format("{0:ddMMMyyyy}", DateSheetCollection[i]);
                            InsertWorksheet(sheetName, tmpFile, i);
                        }
                    }
                    ...
                    if (FormStyles <= 1)
                    {
                        try
                        {
                            ...
                            {
                                GenerateDateSheet(targepck, reader, false, catalog, soldto, DateSheetCollection, AddThresholdQtyComments);
                                targepck.Save();
                            }
                        }
                        catch (Exception ex)
                    ...
                    if (FormStyles > 1)
                    {
                        ...
                        if (reader.GetSchemaTable().DefaultView.Count > 0 && reader["ImageFileName"] != null)
                        {
                            ...
                            try
                            {
                                ...
                                {
                                    GenerateDateSheet(targeImgpck, reader, true, catalog, soldto, DateSheetCollection, AddThresholdQtyComments);
                                    targeImgpck.Save();
                                }
                            }
                            catch (Exception ex)
        ...
        private int GenerateXLSMOrderTemplate(ExcelPackage XLSMPck, string soldto, string catalog, string pricecode, string savedirectory, SqlDataReader xReader)
        {
            ...
            #region "Main tabs"
            var sheet = XLSMPck.Workbook.Worksheets["Order Template"];
            if(sheet == null)
                sheet = XLSMPck.Workbook.Worksheets.Add("Order Template");
                ...
                if (reader.NodeType != XmlNodeType.EndElement && reader.Name == "ProductLevel")
                {
                    ...
                    if (rootDept.Count > 0)
                    {
                        if (rootDept != null && lastRootDeptID != rootDept.FirstOrDefault().Key)
                        {
                            ...
                            deptUPCSheetName = deptUPCSheetName.Length > 22 ? rootDept.FirstOrDefault().Value[0].Substring(0, 22) : deptUPCSheetName;
        ...
        private void GenerateDateSheet(ExcelPackage XLSMPck, SqlDataReader reader, bool withImage, string catalog, string soldto, List<DateTime> DateSheetCollection, bool AddThresholdQtyComments)
        {
            try
            {
                ...
                for (int i = 0; i < DateSheetCollection.Count; i++)
                {
                    ExcelWorksheet dtSheet = null;
                    var StandardDateSheetWithFormControl = ConfigurationManager.AppSettings["StandardDateSheetWithFormControl"];
                    if (StandardDateSheetWithFormControl == "1")
                    {
                        var sheetName = string.Format("{0:ddMMMyyyy}", DateSheetCollection[i]);
                        dtSheet = XLSMPck.Workbook.Worksheets[sheetName];
                        if (ConfigurationManager.AppSettings["column1Imagewidth"] != null)
                            dtSheet.Column(1).Width = Convert.ToInt32(ConfigurationManager.AppSettings["column1Imagewidth"].ToString()) / 5;
                    }
                    else
                        dtSheet = XLSMPck.Workbook.Worksheets.Copy("Order Template", string.Format("{0:ddMMMyyyy}", DateSheetCollection[i]));
        ...
        public void AddLogoToDateSheet(ExcelWorksheet tmpSheet)
        {
        ...
        public void InsertWorksheet(string sheetName, string filePath, int idx)
        {
            // Open the document for editing.
            using (DocumentFormat.OpenXml.Packaging.SpreadsheetDocument spreadSheet = DocumentFormat.OpenXml.Packaging.SpreadsheetDocument.Open(filePath, true))
            {
                //get template
                DocumentFormat.OpenXml.Packaging.WorksheetPart clonedSheet = null;
                var tempSheets = spreadSheet.WorkbookPart.Workbook.GetFirstChild<DocumentFormat.OpenXml.Spreadsheet.Sheets>().Elements<DocumentFormat.OpenXml.Spreadsheet.Sheet>().Where(s => s.Name == string.Format("Order Template", idx));

                var tempPart = (DocumentFormat.OpenXml.Packaging.WorksheetPart)spreadSheet.WorkbookPart.GetPartById(tempSheets.First().Id.Value);
                //copy template to workbook
                var copySheet = DocumentFormat.OpenXml.Packaging.SpreadsheetDocument.Create(new MemoryStream(), DocumentFormat.OpenXml.SpreadsheetDocumentType.Workbook);
                DocumentFormat.OpenXml.Packaging.WorkbookPart copyWorkbookPart = copySheet.AddWorkbookPart();
                DocumentFormat.OpenXml.Packaging.WorksheetPart copyWorksheetPart = copyWorkbookPart.AddPart<DocumentFormat.OpenXml.Packaging.WorksheetPart>(tempPart);
                clonedSheet = spreadSheet.WorkbookPart.AddPart<DocumentFormat.OpenXml.Packaging.WorksheetPart>(copyWorksheetPart);
